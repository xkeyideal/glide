...
func MirrorsList() error {
	...
	for _, r := range ov.Repos {
		if r.Vcs == "" {
			if r.Base != "" {
				msg.Info("--> %s replaced by %s, base: %s", r.Original, r.Repo, r.Base)
			} else {
				msg.Info("--> %s replaced by %s", r.Original, r.Repo)
			}
		} else {
			if r.Base != "" {
				msg.Info("--> %s replaced by %s (%s), base: %s", r.Original, r.Repo, r.Vcs, r.Base)
			} else {
...
func MirrorsSet(o, r, v, b string) error {
	if o == "" || r == "" {
